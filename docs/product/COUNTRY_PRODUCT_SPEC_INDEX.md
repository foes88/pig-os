...
> ⚠ **2026-08-27 결과·한정.** 백엔드 리졸버 축은 통과했다(`test_us_template_lock.py` L1~L6).
> 그러나 **"이 구조 전체" 는 아직 증명되지 않았다** — 모바일이 KPI 목록을 하드코딩하고
> `/kpi/presentation` 을 소비하지 않아, 국가를 데이터로 켜도 모바일에 반영되지 않는다.
> 합격 판정은 **리졸버 축 한정**이다. `docs/MOBILE_PARITY.md` §1-3
