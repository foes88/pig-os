# PigOS — Homepage Section Content
...
```json
{
  "pigplanCore": {
    ...
    "desc": "PigOS는 단순 관리 SaaS가 아닙니다. 데이터가 들어오면 AI가 분석하고, 지금 무엇을 해야 하는지 알려주고, 실행까지 이어지는 양돈 수익 최적화 AI 운영 시스템입니다.",
    "features": {
...
```json
{
  "pigplanCore": {
    ...
    "desc": "PigOS is not just management SaaS. Data flows in, AI analyzes it, tells you what to do right now, and connects to execution. An AI-powered revenue optimization system for pig farms.",
    "features": {
