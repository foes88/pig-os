...
B'. ★ 모바일 DTO 매핑까지 연장 (Android · iOS 각각)
   백엔드 필드 → 모바일 DTO 필드 → 화면 표시값

완료 기준 = A ∩ B ∩ B' 교차 대조.  A 만 하면 오늘과 같은 결과가 나온다.
```

#### B' 를 왜 넣는가 — 모바일이 하드코딩하는 것이 "라벨" 인가 "산식" 인가

모바일이 KPI 목록을 하드코딩한다는 것은 확인됐다(`MOBILE_PARITY` §1-3). 그런데
그것만으로는 **재실사 대상인지 아닌지가 갈리지 않는다.**

```
라벨만 하드코딩          → 백엔드 값을 그대로 받는다 → 재실사 대상 아님
어딘가에서 자체 계산·재매핑 → /kpi/trend 와 똑같은 구조의 오염이
                            Android · iOS 에 각각 존재할 수 있다
```

`/kpi/trend` 사례가 정확히 이 형태였다 — **필드명은 `npd` 인데 값은 WEI**. 같은 일이
모바일 DTO 매핑에서 벌어지면 백엔드만 훑어서는 절대 못 잡는다.

이미 하나 걸려 있다: **iOS 는 알림 severity 를 다시 매칭해 색을 낸다**(Codex C-4).
즉 서버 판정을 그대로 쓰지 않는 계층이 최소 하나 존재한다. 그 계층이 값에도
손대는지는 **확인되지 않았다.**

★ B' 없이 재실사를 끝내면 "전 경로를 안 훑었으므로 더 있을 수 있다" 가 그대로 남는다.
