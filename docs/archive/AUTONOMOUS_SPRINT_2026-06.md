<!-- STATUS: 아카이브 · CLAUDE.md에서 이관(2026-08-05). 역사적 기록 — 현재 작업은 docs/PIGOS_SPEC_INDEX.md 참조. -->

# [ARCHIVE] 2026-06 MVP 자율 스프린트 (Phase 1~14) + 다음 할 일 기록

> CLAUDE.md 본문에서 이관. 6월 MVP 스프린트(54/54 완료) 상세 계획·체크리스트.
...
- [x] **[P4-6] /record 모바일 최적화**
  - md 이하에서 좌우 분할 레이아웃 → 스택(위아래) 레이아웃으로 전환
  - 모돈 검색을 상단 고정 drawer 또는 모달로 변경

---

### Phase 5 — i18n (현재 8개 언어)

> **주의**: 초기 5개 언어(en/ko/zh/es/vi)에서 **8개 언어로 확장됨** (en/ko/zh/es/vi/th/pt/ru — 2026-07 이후).
> 새 UI 텍스트는 `src/messages/` 아래 **8개 파일 모두** 동시 업데이트 필수 (규칙 4, `i18n.test.ts` 키 파리티 강제).
> ※ ko는 판매 타겟 아닌 대표 검토용 로케일(상단 개요 참조).
...
- [x] **[P9-4] Sidebar 메뉴 최종 정비** (⚠️ 2026-06-10 부분 완료: lucide 아이콘 + 공식 로고 + 그룹핑(대시보드/돈군관리/기록/분석/Addon) + 다국어 현지 용어 + active 하이라이트 적용됨. `src/components/Sidebar.tsx` 먼저 읽고 기존 구조 유지할 것 — 전면 재작성 금지)
