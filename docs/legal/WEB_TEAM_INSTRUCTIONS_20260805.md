...
### ✅ 2026-08-26 확정 — 개인정보 연락처는 `wiselake@wiselake.ai` **하나**다

전 문서(방침 제1조 보호책임자·담당부서, 부속조항 6벌, 권리행사 창구, footer)가 같은 주소를 쓴다.

★ 2026-08-25 에 잠시 "보호책임자는 개인 메일, 담당부서는 조직 메일"로 나눴다가 되돌렸다.
  PIPA §30 은 **'연락처'를 요구할 뿐 개인 이메일을 요구하지 않는다.** 조직 메일이 낫다:
  (a) 담당자가 바뀌어도 방침을 고치지 않아도 된다
  (b) 공개 문서(웹·앱·스토어)에 개인 이메일을 노출하지 않는다
  (c) 이미 실제 수신·운영 중임이 확인됐다

폐기 대상은 `gyomoon@ezfarm.co.kr`(구 도메인)뿐이고, forwarding 은 유지한다.
