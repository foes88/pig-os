...
5. **사용자 노출 기능/수치가 바뀌면** `docs/LANDING_SYNC.md`(랜딩 동기화 정본)를 갱신. 랜딩페이지(pigos.io, `c:/dev/pigos-landing`)는 이 문서를 참조해 `FACTS.md`를 맞춘다. **위조 0**: `[확정]`만 랜딩에 사실로 노출, `[베타/검증중]`은 hedge, `[내부]`는 비노출.
...
- **타겟 시장**: **5개 글로벌 시장 (US · CN · SEA[VN+TH] · LatAm · KR) × 8개 언어** — LANDING_SYNC §3 [확정], 로그인 문구 "5 global markets · 8 languages"
  - **KR = 레퍼런스 전용** (대표 확인 2026-08-05): 공개 마케팅 타겟 아님. 단 **대표 확인·검토용 접근은 유지**(가입 하드차단·안내문구 미적용 — 대표 요청). 한국법(PIPA·준거법·법정보존)은 회사 본국법으로 계속 적용. ko 로케일=대표 검토용
