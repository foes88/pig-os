...
async def _born_alive_low(ctx: RuleContext) -> list[Finding]:
    v = ctx.kpi.get("BORN_ALIVE")
    if v is None:
        return []
    w, c = resolve(ctx, "born_alive.low", "BORN_ALIVE", 11.0, 10.0)
    sev = sev_below(v, w, c)
    if sev is None:
        return []
    ...
    return [_finding("born_alive.low", "BORN_ALIVE", sev, v, w, causes, actions)]
...
async def _weaned_low(ctx: RuleContext) -> list[Finding]:
    ...
    if v is None:
        return []
    w, c = resolve(ctx, "weaned.low", "WEANED_COUNT", 10.0, 9.0)
    sev = sev_below(v, w, c)
    if sev is None:
        return []
    ...
    return [_finding("weaned.low", "WEANED_COUNT", sev, v, w, causes, actions)]
...
async def _birth_weight_low(ctx: RuleContext) -> list[Finding]:
    v = ctx.kpi.get("BIRTH_WEIGHT")
    if v is None:
        return []
    w, c = resolve(ctx, "birth_weight.low", "BIRTH_WEIGHT", 1.3, 1.1)
    sev = sev_below(v, w, c)
    if sev is None:
        return []
    ...
    return [_finding("birth_weight.low", "BIRTH_WEIGHT", sev, v, w, causes, actions)]
...
async def _weaning_weight_low(ctx: RuleContext) -> list[Finding]:
    v = ctx.kpi.get("WEANING_WEIGHT")
    if v is None:
        return []
    w, c = resolve(ctx, "weaning_weight.low", "WEANING_WEIGHT", 5.5, 5.0)
    sev = sev_below(v, w, c)
    if sev is None:
        return []
    causes = ["low_weaning_weight"]
    actions = ["improve_lactation_feed_and_creep", "review_sow_milk_yield"]
    return [_finding("weaning_weight.low", "WEANING_WEIGHT", sev, v, w, causes, actions)]
...
async def _lactation_short(ctx: RuleContext) -> list[Finding]:
    v = ctx.kpi.get("WEANING_AGE")
    if v is None:
        return []
    w, c = resolve(ctx, "lactation.too_short", "WEANING_AGE_LOW", 19.0, 16.0)
    sev = sev_below(v, w, c)
    if sev is None:
        return []
    causes = ["lactation_length_too_short"]
    actions = ["adjust_weaning_age_to_target", "review_early_weaning_protocol"]
    return [_finding("lactation.too_short", "WEANING_AGE", sev, v, w, causes, actions)]
...
async def _lactation_long(ctx: RuleContext) -> list[Finding]:
    ...
    if v is None:
        return []
    w, c = resolve(ctx, "lactation.too_long", "WEANING_AGE_HIGH", 28.0, 35.0)
    sev = sev_above(v, w, c)
    if sev is None:
        return []
    causes = ["lactation_length_too_long"]
    actions = ["adjust_weaning_age_to_target", "review_sow_throughput_and_npd"]
    return [_finding("lactation.too_long", "WEANING_AGE", sev, v, w, causes, actions)]
